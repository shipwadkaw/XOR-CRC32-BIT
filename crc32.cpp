...
#define DOONCE crc = crc32_table[((int)crc ^ (*input++)) & 0xff] ^ (crc >> 8)
extern bool GetDLCFilename(const char *filename, char *filename_out, int len);

unsigned long getCrc32(unsigned long crc, const unsigned char *input, unsigned len)
{
	if (input == 0) return 0;
	crc = crc ^ CRC_CODE;
    while(len >= 8) 
    {
		DOONCE; DOONCE; DOONCE; DOONCE; DOONCE; DOONCE; DOONCE; DOONCE;
        len -= 8;
    }
	if (len == 0) return crc ^ CRC_CODE;
    do { DOONCE;}while(--len);
	return crc ^ CRC_CODE;
}
...
{   
	...
	fread(buf,size,1,fileptr);
	res = getCrc32(CRC_CODE, buf, size);
    free(buf);
	fclose(fileptr);
    return res;
} 
